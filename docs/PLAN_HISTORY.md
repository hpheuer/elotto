...
**This file is closed. Nothing here is a live instruction.** It is the design work and the
measured findings for Task 1 (per-loop camera calibration) up to the point where the task was
finished and its control passed. The live contract is [`PLAN.md`](PLAN.md), which continues at
**§1.15**.

⚠ **Section numbers are deliberately unchanged.** CLAUDE.md, README.md and several source
comments cite `§1.5`, `§1.10`, `§1.12`, `§1.13` and others by number. Renumbering would break
every one of those citations silently, which is exactly the failure this project already carries
from the deleted `PLAN_4NODE.md`. So:

| citation | lives in |
|---|---|
| §1.1 – §1.14 | **this file** |
| §1.15 onward | [`PLAN.md`](PLAN.md) |

⚠ Read findings here as **records of what was true when measured**, not as current state. Several
have since been overturned by later sections — §1.9's bias-vs-exposure curve was largely a curve
in *light* (§1.11), the sealed-dark enclosure of §1.11–1.12 was subsequently **lit** (§1.13), and
the `mean_px` gate and selection rule both flagged as "not changed" here were changed on
2026-07-27/28 (§1.16, §1.17). Check `CLAUDE.md`'s "Where things stand" before acting on anything
in this file.
