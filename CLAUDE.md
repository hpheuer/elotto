...
or the next unlimited round — recalibrates every node including that one. It returns after four
clean blocks. ⚠ `?calint=0` skips mid-session sweeps; a trip then waits until the next session.
...
anything. The sweep that follows the block close (calint insertion, or the next round) is what
...
fault). `h1`/`h2` are z of the two halves of the same window `[D56]`; a slave without them
...
⚠ STATUS / PLAN_HISTORY are history, not rules. Everything above this line is the rule set.
